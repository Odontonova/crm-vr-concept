...
export default function Conversations() {
  ...
  // Estado para controlar a abertura da barra lateral
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  // Estado para controlar qual aba está ativa (mensagens rápidas ou IA)
  const [sidebarTab, setSidebarTab] = useState<'mensagens' | 'agente'>('mensagens');

  // Fecha o painel ao mudar para mobile se estiver aberto
  useEffect(() => {
    if (isMobile) {
  ...
  return (
    <div className="h-[calc(100vh-4rem)]">
      ...
      <div className="flex h-full w-full lg:rounded-lg lg:border bg-background overflow-hidden relative">
        ...
        {/* Painel Direito Desktop: Mensagens Rápidas / IA (Lateral) */}
        {!isMobile && isSidebarOpen && leadId && (
          <div className="hidden lg:block h-full flex-shrink-0 border-l bg-card w-72 xl:w-80 2xl:w-96 transition-all duration-300">
            <QuickMessagesSidebar 
              lead={lead || null} 
              activeTab={sidebarTab}
              onTabChange={(tab) => setSidebarTab(tab as 'mensagens' | 'agente')}
              onClose={() => setIsSidebarOpen(false)}
            />
          </div>
        )}

        {/* Painel Mobile: Mensagens Rápidas / IA (Gaveta Inferior) */}
        {isMobile && leadId && (
